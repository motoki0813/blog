...
## ロードマップ
既存 MRP エンジンでは以下のロードマップが 10.0.32 以降適用されます。  
...
## テクニカル サポートの対応について
既存 MRP エンジンに関するお問い合わせを起票いただく際には、以下の内容をご確認・ご理解ください。
...
## ロードマップ
計画最適化では以下のロードマップが 10.0.32 以降適用されます。
- 既存 MRP エンジンとの差分機能や設定については、新規お問い合わせにて計画最適化への追加希望をご依頼いただくことで実装可否を検討するロードマップへ追加されます
- 計画最適化内部でのカスタマイズは基本的に実装できないため、お客様の運用を計画最適化に併せていただくことを検討ください
- カスタマイズを希望する箇所がある際には、Ideas に投稿ください  
